...
class ExportReportIrsaControllerCNAM(http.Controller):
    ...
    def writePayslip(self, workbook, worksheet, row, col, payslip, total):
        ...
        worksheet.write(row, col + 4, '{:,.2f}' .format(round(basic2,2)), border_black)
        worksheet.write(row, col + 5, '{:,.2f}' .format(round(prm,2)), border_black)
        worksheet.write(row, col + 6, '{:,.2f}' .format(round(hs,2)), border_black)
        worksheet.write(row, col + 7, '{:,.2f}' .format(round(payslip.employee_id.remaining_leaves,2)), border_black)
        worksheet.write(row, col + 8, '{:,.2f}' .format(round(preavis,2)), border_black)
        worksheet.write(row, col + 9, '{:,.2f}' .format(round(gross,2)), border_black)
        worksheet.write(row, col + 10, '{:,.2f}' .format(round(cnaps_emp,2)), border_black)
        worksheet.write(row, col + 11, '{:,.2f}' .format(round(omsi_emp,2)), border_black)
        worksheet.write(row, col + 12, '{:,.2f}' .format(round(ostie_emp,2)), border_black)
        worksheet.write(row, col + 13, '{:,.2f}' .format(round(osief_emp,2)), border_black)
        worksheet.write(row, col + 14, '{:,.2f}' .format(round(divers,2)), border_black)
        worksheet.write(row, col + 15, '{:,.2f}' .format(round(net,2)), border_black)
        worksheet.write(row, col + 16, '{:,.2f}' .format(round(mimpo,2)), border_black)
        worksheet.write(row, col + 17, '{:,.2f}' .format(round(irsa,2)), border_black)
        worksheet.write(row, col + 18, '{:,.2f}' .format(round(enfant,2)), border_black)
        worksheet.write(row, col + 19, '{:,.2f}' .format(round(impnet,2)), border_black)
    ...
    def body(self, workbook, worksheet, payslips, total, total_net_cumul):
        ...
        worksheet.write(row + 1, col, '{:,.2f}' .format(round(total['basic2'], 2)), border_black)
        worksheet.write(row + 1, col + 1, '{:,.2f}' .format(round(total['prm'],2)), border_black)
        worksheet.write(row + 1, col + 2, '{:,.2f}' .format(round(total['hs'],2)), border_black)
        worksheet.write(row + 1, col + 3, '{:,.2f}' .format(round(total['conge'],2)), border_black)
        worksheet.write(row + 1, col + 4, '{:,.2f}' .format(round(total['preavis'],2)), border_black)
        worksheet.write(row + 1, col + 5, '{:,.2f}' .format(round(total['gross'], 2)), border_black)
        worksheet.write(row + 1, col + 6, '{:,.2f}' .format(round(total['cnaps_emp'], 2)), border_black)
        worksheet.write(row + 1, col + 7, '{:,.2f}' .format(round(total['omsi_emp'],2)), border_black)
        worksheet.write(row + 1, col + 8, '{:,.2f}' .format(round(total['ostie_emp'],2)), border_black)
        worksheet.write(row + 1, col + 9, '{:,.2f}' .format(round(total['osief_emp'],2)), border_black)
        worksheet.write(row + 1, col + 10, '{:,.2f}' .format(round(total['divers'],2)), border_black)
        worksheet.write(row + 1, col + 11, '{:,.2f}' .format(round(total['net'],2)), border_black)
        worksheet.write(row + 1, col + 12, '{:,.2f}' .format(round(total['mimpo'],2)), border_black)
        worksheet.write(row + 1, col + 13, '{:,.2f}' .format(round(total['irsa'],2)), border_black)
        worksheet.write(row + 1, col + 14, '{:,.2f}' .format(round(total['enfant'],2)), border_black)
        worksheet.write(row + 1, col + 15, '{:,.2f}' .format(round(total['impnet'],2)), border_black)

        # head
        total_net_period = total['net'] - total['impnet']
        worksheet.write('D8', '{:,.2f}' .format(total_net_cumul + total['net']), workbook.add_format({'top': 1, 'right': 1, "bg_color": "red"}))
        worksheet.write('D11', '{:,.2f}' .format(total_net_period), workbook.add_format({'right': 1, "bg_color": "red"}))
        worksheet.write('D13', '{:,.2f}' .format(total_net_cumul + total_net_period + total['net']), workbook.add_format({'top': 1, 'right': 1, "bg_color": "red"}))
