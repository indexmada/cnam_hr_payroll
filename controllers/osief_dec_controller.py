...
class ExportReportOsiefController(http.Controller):
    ...
    def report_excel_employer(self, workbook, sante, plf, y, trim, eff, row_count):  #
        ...
        for employee in payslips.mapped('employee_id'):
            ...
            for payslip in pslips:
                ...
                if month_trim == 1:
                    ...
                    worksheet_ost.write('E'+str(line), '{:,.2f}' .format(sbr), cust_12)
                    ...
                    if month_trim == 2:
                        sal_2 += sbr
                        worksheet_ost.write('H'+str(line), '{:,.2f}' .format(sbr), cust_12)
                    elif month_trim == 3:
                        sal_3 += sbr
                        worksheet_ost.write('K'+str(line), '{:,.2f}' .format(sbr), cust_12)
        ...
        # Sal_1
        cell = 'E'+str(line)
        worksheet_ost.write(cell, '{:,.2f}' .format(sal_1), cell_right_12)
        cell = 'E'+str(line+1)
        ...
        # Sal_2
        cell = 'H'+str(line)
        worksheet_ost.write(cell, '{:,.2f}' .format(sal_2), cell_right_12)
        cell = 'H'+str(line+1)
        worksheet_ost.write(cell, '', cell_right_12)
        cell = 'H'+str(line+2)
        worksheet_ost.write(cell, '{:,.2f}' .format(sal_2), cell_right_12)
        ...
        # Sal_3
        cell = 'K'+str(line)
        worksheet_ost.write(cell, '{:,.2f}' .format(sal_3), cell_right_12)
        cell = 'K'+str(line+1)
        worksheet_ost.write(cell, '', cell_right_12)
        cell = 'K'+str(line+2)
        worksheet_ost.write(cell, '{:,.2f}' .format(sal_3), cell_right_12)
        ...
        val = 'Cotisation :……'+'{:,.2f}' .format(cotisation)+'  Ariary                        Total Salaire……'+'{:,.2f}' .format(total_sal)+' Ariary'
        ...
        line += 1
        cell = 'A'+str(line)
        val = 'NET A PAYER : (chiffres)…'+'{:,.2f}' .format(cotisation)+' Ariary…… ; '
